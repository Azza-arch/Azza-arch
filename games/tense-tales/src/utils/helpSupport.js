(function(){
  ...
  function open(key){var c=content[key]||content.storyOrder,p=document.getElementById('learning-help');p.querySelector('h2').textContent=c.title;p.querySelector('[data-help-en]').textContent=c.en;p.querySelector('[data-help-bm]').textContent=c.bm;p.hidden=false;document.getElementById('help-close').focus();}
  function close(){var p=document.getElementById('learning-help');p.hidden=true;}
  function bind(){document.addEventListener('click',function(e){var b=e.target.closest('[data-help]');if(b)open(b.dataset.help);});document.getElementById('help-close').addEventListener('click',close);document.addEventListener('keydown',function(e){if(e.key==='Escape')close();});}
