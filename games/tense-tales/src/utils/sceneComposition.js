(function () {
  ...
  var environmentPresets={
    ...
    studyRoom:[surface('wall','#f4e5c7',0,84,0),surface('baseboard','#efe8d9',81,3,1),surface('floor','#c9955d',84,16,1),image('window','studyWindow',15,10,22,1,'top-left'),image('bookcase','studyBookcase',88,83,15,1),image('lamp','studyFloorLamp',73,83,8,1),image('chair','studyChair',45,84,14,2),image('desk','studyDesk',50,88,45,4)]
  };
  var ratios={ characterKidMale:.75,characterKidFemale:.75,objectPhone:.58,objectKeys:1.2,objectPencil:3.2,objectBookOpen:1.35,objectBookClosed:.78,objectFolder:1.15,objectBackpack:.78,objectLaptop:1.4 };
  ...
  function boundsFor(layer,frame,actorRef){
    ...
    var ratio=ratios[layer.assetId]||1, w,h;
  ...
  function resolveScene(scene,frame,catalog,context){
    ...
    defs.filter(function(l){return l.role==='actor';}).forEach(function(l){l.boundsPx=boundsFor(l,frame);byId[l.id]=l;});
    ...
    defs.forEach(function(l){
      ...
      l.boundsPx=boundsFor(l,frame,actorRef);byId[l.id]=l;
    });
